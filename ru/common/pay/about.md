...
Вы можете отменить заявку на вывод средств, если операция ещё не была обработана (статус В обработке). Для таких операций в блоке **{{ ui_worker_prfl-money-history }}** будет доступна кнопка **{{ ui_worker_worker-transaction-canceling__button__title }}**.
