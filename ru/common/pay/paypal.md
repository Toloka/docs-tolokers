...
{% endif %}
1. Найдите платежную систему PayPal и нажмите кнопку **{{ ui_worker_money__withdraw__submit }}**.
1. Привяжите ваш кошелек PayPal. Кошелек должен быть верифицирован. Привязанный кошелек будет отображаться в разделе {% if platform=="web" %}**{{ ui_worker_prfl-tab-money }}**{% elsif platform=="android" %}**{{ mobile_android_money_main_title }}**{% elsif platform=="ios" %}**{{ mobile_ios_profile_my_money_section_header }}**{% endif %}.
...
1. В поле **{{ ui_worker_wthdrw-amnt }}** введите необходимую сумму и нажмите кнопку **{{ ui_worker_money__withdraw__submit }}**.
