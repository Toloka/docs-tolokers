...
{% if platform=="web" %}
[Авторизируйтесь](register.md) в Толоке и откройте страницу [Задания](https://toloka.yandex.ru/tasks). По умолчанию сначала отображаются избранные задания. Чтобы установить другой порядок сортировки, вверху над списком выберите один из критериев:
...
Недоступные задания отмечены значком ![](assets/lock.svg). Рядом указано условие доступа к заданию. Например, условие {% if locale=="ru-com" %}**Мобильное iOS или Android приложение**{% elsif locale=="en-com" %}**iOS or Android app**{% endif %} означает, что задание нельзя выполнить в полной версии Толоки, только в мобильном приложении.

## Как фильтровать задания {#filter-tasks}

Задания можно фильтровать по критериям: 

-    **{{ ui_worker_tasks_with_training }}** — для доступа к таким заданиям необходимо пройти обучение.
-    **{{ ui_worker_tasks_with_non-automatic-acceptance }}** — будут оплачены только после проверки заказчиком.
-    **{{ ui_worker_tasks_adult }}** — содержат контент для взрослых.
-    **{{ ui_worker_tasks_unavialable }}** — для доступа к таким заданиям необходимо выполнить определенные условия.
-    **[{{ ui_worker_tasks_hidden }}](#hide-task)** — задания, которые вы исключили из общего списка.

Также вы можете фильтровать задания по категориям. Например:
...
Можно выбирать задания от определенных заказчиков. 
...
- Выберите **{{ ui_worker_task_menu__bookmark_add }}**.
         Или нажмите значок ![](assets/favourites.png =15x) рядом с названием задания.
