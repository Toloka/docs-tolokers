...
{% if platform=="web" %}
Авторизуйтесь в Толоке и откройте страницу [Задания](https://toloka.yandex.ru/tasks). По умолчанию сначала отображаются избранные задания. Чтобы установить другой порядок сортировки, вверху над списком заданий выберите один из критериев:
...
Задания можно фильтровать по видам: 

-    **{{ ui_worker_tasks_with_training }}** — для доступа к таким заданиям необходимо пройти обучение.
-    **{{ ui_worker_tasks_with_non-automatic-acceptance }}** — задания, которые будут оплачены только после проверки заказчиком.
-    **{{ ui_worker_tasks_adult }}** — задания, содержащие контент для взрослых.
-    **{{ ui_worker_tasks_unavialable }}** — задания, для доступа к которым необходимо выполнить определенные условия.
-    **{{ ui_worker_tasks_hidden }}** — задания, которые вы исключили из общего списка.
...
Кроме того, вы можете выбирать задания от определенных заказчиков. 
