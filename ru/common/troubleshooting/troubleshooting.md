...
Если вы не хотите выполнять задание из списка, не приступайте к нему. Если вы начали делать задание, но не можете или не хотите выполнять открывшуюся страницу, перейдите к следующей (кнопка {% if platform=="web" %}**{{ ui_worker_new-task-page__skip-button }}**{% elsif platform=="android" %}**{{ mobile_android_task_skip }}**{% elsif platform=="ios" %}**{{ mobile_ios_task_actions_skip }}**{% endif %}). Оплачиваются только полностью выполненные задания.
