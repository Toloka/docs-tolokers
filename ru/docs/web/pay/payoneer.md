...
1. На главной странице официального сайта [Payoneer]({{ payoneer }}) нажмите кнопку **Зарегистрироваться**.
1. Пройдите небольшой опрос, для каких целей вы планируете использовать систему **Payoneer**.
    
    {% cut "Пример" %}
	
	![](../images/Payoneer/Payoneer-registration_rus_1.png)
...
1. Кошелек привязывается один раз, после чего будет доступен вывод средств во вкладке {% if platform=="browser" %}**{{ ui_worker.prfl-tab-money }}**{% elsif platform=="android" %}**{{ mobile_android.money_main_title }}**{% elsif platform=="ios" %}**{{ mobile_ios.profile.my_money_section_header }}**{% endif %}.
1. Введите необходимую сумму и нажмите кнопку **{{ ui_worker.money__withdraw__submit }}**.
1. На телефон придет SMS с кодом. Введите его и нажмите кнопку {% if platform=="browser_needed" %}**{{ ui_worker.sms_popup__submit }}**{% elsif platform=="ios" or platform=="android" %}**{{ mobile_android.button_confirm_sms }}**{% endif %}.
